...
app.use((req, res, next) => {
  ...
  res.setHeader(
    ...
    [
      ...
      // noria-engine.onrender.com — NORIA Pro's dedicated AI/billing API. *.paddle.com —
      // Paddle.js checkout (subscriptions).
      "connect-src 'self' https://api.groq.com https://api.cerebras.ai https://generativelanguage.googleapis.com https://*.supabase.co https://api.anthropic.com https://skyra-data-backend.fly.dev https://noria-engine.onrender.com https://*.paddle.com http://localhost:*",
      // Allow our own pages (e.g. the showreel) to be embedded in same-origin
      // iframes, YouTube video embeds in the homepage video panel, and Paddle's
      // checkout overlay (NORIA Pro subscriptions).
      "frame-src 'self' https://skyra-data-backend.fly.dev https://www.youtube-nocookie.com https://www.youtube.com https://youtube.com https://*.paddle.com",
...
// NORIA Pro — sign in / pricing (weekly or monthly) / premium chat, talking
// directly to the dedicated noria-engine so the paywall it enforces actually
// gates access. Separate from the free /noria/chat assistant above.
app.get(['/noria/pro', '/noria/pricing', '/noria/subscribe'], (req, res) => res.sendFile(path.join(__dirname, 'noria-pro.html')));
...
// Generic Apply & Pay form — the only frontend that reaches /api/apply's
// SERVICE_PRODUCT_MAP-priced services (visas, admissions, bookings, letters…),
// which previously had no page pointing at it at all.
...
const SITE_PAGES = [
  ...
  { title: 'NORIA Pro', url: '/noria/pro', desc: 'Subscribe weekly or monthly for NORIA’s premium AI models.', tags: 'noria pro subscribe subscription pricing membership premium plan weekly monthly pay billing' },
  ...
  { title: 'Apply & Pay', url: '/apply', desc: 'Apply for a visa, admission, booking or document service and pay online.', tags: 'apply pay visa admission booking flight hotel insurance scholarship letter checkout payment' },
...
app.post('/api/pay/init', async (req, res) => {
  try {
    ...
    let amount = isCrypto ? prod.USD : prod[cur];
    ...
    // SkyGlobeGroup Members get 10% off every service — server-authoritative,
    // same tamper-proof pricing rule as everything else here.
    const isMember = await isActiveMember(email);
    if (isMember) amount = Math.round(amount * 0.9 * 100) / 100;

    const reference = genPayRef();
    await insertPayment({
      reference, product, provider, currency: cur, amount,
      email, app_ref: app_ref || null, status: 'pending', meta: { ...(meta || {}), memberDiscount: isMember || undefined },
    });
